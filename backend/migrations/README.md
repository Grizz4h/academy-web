...
| `006_competency_persistence.sql` | Phase 4C.1 — evidence events + derived competency states |
...
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -f backend/migrations/006_competency_persistence.sql
```
...
cd backend && python3 test_schema_competency.py
...
- `005_admin_audit_log.sql`: Audit-Log für geschützte operative Admin-Aktionen.
